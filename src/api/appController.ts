...
/** 此处后端没有提供注释 GET /app/build/status/stream */
export async function streamBuildStatus(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.streamBuildStatusParams,
  options?: { [key: string]: any }
) {
  return request<API.SseEmitter>('/app/build/status/stream', {
    method: 'GET',
    params: {
      ...params,
    },
